...
Fabric is the first implementation of Atlas (Atlas §30). It is an application, not an operating
system: a hosted Atlas runtime plus a desktop showcase that makes the invisible — authority,
delegation, provenance — visible and manipulable. Its purpose is to produce evidence about the
Atlas model in both directions: where the model carries real weight, and where it creaks.

Two deliverables share one codebase:

- **Fabric runtime** — a faithful, reusable implementation of Atlas Base semantics as a .NET
  library. This is the part that must be *correct*.
- **Fabric Studio** — an Avalonia desktop application hosting showcase scenes on top of the
...
specification itself; Fabric should not run ahead of it. Atlas §6.8 cuts both ways — the spec
must not become Fabric-shaped, and Fabric must not pretend to be further along than the spec.

Fabric targets Architecture 0.4 as one complete baseline. Requirements stated normatively in 0.4
...
Fabric.sln
├── src/
│   ├── Fabric.Core                   — Atlas Base: domains, actor references, capabilities,
│   │                                   delegation, operations, executions, events, outcomes,
│   │                                   shapes, genesis, origin, provenance log
│   ├── Fabric.Extensions.Events      — Event Distribution (Atlas §19.2): mediator, subscriptions
│   ├── Fabric.Extensions.Flow        — Flow (Atlas §19.1): items, cursors, recovery contract
│   ├── Fabric.Experimental.Enrichment— targeted Enrichment experiments (Atlas §16.6)
│   ├── Fabric.Vocabularies.Cooling   — the Atlas §17 demo vocabulary (Temperature, Fan)
│   ├── Fabric.Vocabularies.Input     — Input.Pointer (the Atlas §21.1 template, once drafted)
│   └── Fabric.Studio                 — Avalonia app: scenes + inspectors
└── tests/
    ├── Fabric.Conformance            — the spec, executable (see §5 below)
    ├── Fabric.Enrichment.Tests       — explicitly non-conformance experiments for Atlas §16.6
    └── Fabric.Core.Tests             — implementation-level tests
```
...
must never leak into Core — Fabric must not define Atlas, and Studio must not define Fabric.

## 3. Mapping Atlas onto .NET
...
**Experimental Enrichment (§16.6).** Enrichment is not part of `Fabric.Core`. The first experiment
...
prove a Fabric domain can be all compile-time tables), and a NativeAOT headless "microdomain"
sample.
...
1. **M0 — Skeleton and executable spec.** Solution scaffold with dependency rules enforced by
   project references. Encode Atlas §29.2 (the delegation Given/When/Then and the Shape
...
3. **M2 — Cooling scene, headless.** The Atlas §17 system end-to-end: sensor emits, controller
...
4. **M3 — Shape composition.** The §16 contract green: additive same-name versions, open-record
   ...
   `Bob:DirectionalVelocity` cases pass. Mirrors Linen's M3, so the interchange experiment later
   meets a like-for-like Shape surface.
5. **M4 — Targeted Enrichment experiment.** Implement the Atlas §16.6 pointer-temperature case
...
Every conformance test cites the section it enforces (`[SpecSection("11")]`). When Fabric and the
spec disagree, the disagreement itself is the deliverable: either the code is wrong, or we have
found a spec bug — and both feed the next Atlas revision's change log. Architecture 0.4's current
change log is §36. This is Fabric's actual purpose per Atlas §30: evidence, not just software.
...
- **Experimental leakage.** Enrichment convenience entering Core would turn Fabric's experiment
  into an accidental Atlas requirement. The project boundary, separate test suite, and explicit
...
by directory names or prior claims. If no Fabric implementation exists, use the sequence below.

### 8.1 Repository preparation

1. Read `Atlas-Architecture-0.4.md` completely, then this plan, before designing public types.
...
3. Create only the M0 projects initially:
   ...
   ```text
   src/Fabric.Core
   src/Fabric.Vocabularies.Cooling
   tests/Fabric.Conformance
   tests/Fabric.Core.Tests
   ```

4. Enforce the dependency direction through project references. `Fabric.Core` has no project-local
...
5. Record the exact .NET SDK and test framework selected. Avoid additional dependencies unless
   they remove substantial accidental work without defining Atlas semantics on Fabric's behalf.
...
- tests cite their Atlas section through a small `SpecSection` mechanism; and
...
- Fabric and the tests agree because of Atlas contracts, not shared CLR types.

### 8.5 Targeted Enrichment experiment

Only after M3, add `Fabric.Experimental.Enrichment` and `Fabric.Enrichment.Tests`. The experimental
layer wraps Core and constructs a complete Operation input before invoking the Core Execution gate;
`Fabric.Core` remains unaware of Enrichment. This ordering is itself an experimental hypothesis,
not a new Atlas rule.
...
1. `Atlas-Architecture-0.4.md` for Atlas semantics;
2. this plan for Fabric sequencing and implementation boundaries;
3. section-cited conformance tests as executable interpretations of the Architecture; and
4. existing Fabric code as an implementation that may be corrected.
...
as an Atlas ambiguity, plan gap, test defect, or Fabric defect.

Architecture §16.6 is deliberately work in progress. Its cases belong to the experimental suite,
not normative conformance. Experimental success is evidence for Atlas; it is not permission to
move Enrichment into Base or Core.

### 9.2 Working rules for Codex

When delegated Fabric synthesis, Codex should:
...
- keep Fabric-specific convenience and all §16.6 experiments outside `Fabric.Core`;
...
Each finding should contain the Atlas section, minimal scenario, observed result, expected result,
classification, and current disposition. Valid classifications are `Fabric defect`, `test defect`,
`plan gap`, `Atlas ambiguity`, and `experimental result`.

Codex should not:

- edit Atlas Architecture to make an implementation pass unless the user separately requests a
  specification change;
- begin Linen, Studio, networking, persistence, distributed authority, or a later milestone merely
...
- use a shared CLR model as though it were the Atlas contract;
...
It should pause for the user when a choice changes Atlas-visible semantics, crosses the Base versus
experimental boundary, requires a materially broader scope, or would make future Fabric/Linen
interchange depend on a Fabric-private convention.
...
The following instruction may be given to Codex from the Fabric repository:

```text
Synthesize Fabric according to Atlas-Architecture-0.4.md and
Fabric-Implementation-Plan-0.2.md.
...
Preserve Atlas-visible semantics over framework convenience. Keep one checked Execution path,
make authority fail closed, preserve unrelated user changes, and record material discrepancies in
docs/implementation-findings.md. Do not edit the Atlas Architecture, start Linen, or advance to
Studio or later milestones before the current gate is complete.

Treat Atlas §16.6 as experimental: keep Enrichment outside Fabric.Core and normative conformance;
...
unresolved choice would change Atlas-visible semantics, cross the Base/experimental boundary,
depend on a Fabric-private convention for interchange, require broader authority, or contradict
