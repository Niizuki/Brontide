...
Fabric now retains an experimental, Fabric-owned host adapter and provider endpoint for the neutral
Cooling fixture. It exchanges process data only and does not alter the normative status of Fabric
...
| Interchange P0-P4 | Neutral fixtures; independent v2 manifest/value/message implementations; Fabric host adapter and provider endpoint; real two-way foreign-process tests; dependency/output audit | Green experimental cross-stack evidence; protocol and observation format remain unratified |
...
The Architecture 0.5 image workspace still supplies Fabric-local evidence only. A real Linen
Cooling Component now interchanges with Fabric, but the mixed image workspace and cross-machine or
...
authority and unknown Constraints fail closed in the Fabric host; the required host-context
