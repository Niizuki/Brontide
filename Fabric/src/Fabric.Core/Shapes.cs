...
public static class BuiltInShapes
{
    ...
    public static readonly ShapeReference OperationSet = ShapeReference.Parse("Fabric:OperationSet", 1);
    public static readonly ShapeReference TimeWindow = ShapeReference.Parse("Fabric:TimeWindow", 1);
    public static readonly ShapeReference Lease = ShapeReference.Parse("Fabric:Lease", 1);
    public static readonly ShapeReference OriginClass = ShapeReference.Parse("Fabric:OriginClass", 1);
    public static readonly ShapeReference Details = ShapeReference.Parse("Fabric:Details", 1);
    public static readonly ShapeReference Activity = ShapeReference.Parse("Fabric:Activity", 1);
}
...
public sealed class ShapeRegistry
{
    ...
    public void Register(DeclaredFragmentDefinition definition)
    {
        ...
        lock (_gate)
        {
            ...
            if (definition.IsAuthoredAttachment)
            {
                ...
                if (hostDefinition.Kind != ShapeKind.Record || hostDefinition.FragmentPolicy != Fabric.Core.FragmentPolicy.Open)
                {
    ...
    private ShapeProjectionResult ProjectRecord(ShapeValue value, ShapeDefinition accepted, ShapeContract contract)
    {
        ...
        foreach (var presented in record.Fragments)
        {
            ...
            {
                if (accepted.FragmentPolicy == Fabric.Core.FragmentPolicy.Closed)
                {
            ...
            if (fragment.IsAuthoredAttachment && accepted.FragmentPolicy == Fabric.Core.FragmentPolicy.Closed)
            {
                return ShapeProjectionResult.Invalid($"Closed Shape {accepted.Reference} rejects authored Fragments.");
            }
