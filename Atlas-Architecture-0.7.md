...
The status of every named Atlas concept is recorded here once, replacing per-section status
disclaimers. The registry is an index; normative force remains with each concept's defining
section or design note.

| Term | Status |
| --- | --- |
| Actor, Capability, Shape, Delegation, Operation, Execution, Event, Outcome | Atlas Base (eight terms) |
...
| Channel, Resource, Composition, Discovery, Runtime, Topology, Distributed, Identity, Persistence, Realtime, Presentation, Workspace, Intent, State, Transaction, Lifecycle, Time | Provisional extension names (§19); no accepted extensions implied |
...
> `Atlas-Design-Note-Enrichment-0.1.md`. It is not a normative Atlas mechanism, adds no Base
...
- Obtaining a value is an Operation. Capability invocation, sensor reads, provider queries,
  and other effectful acquisition must remain visible as ordinary Atlas Operations and must
...
- Systems are not necessarily topological. Graphs describe particular views — an Execution
  trace, a deployment composition, a Flow — rather than the universal form of an Atlas
  system.
...
> hot swapping, the proposed Atlas Portable Binding, representation mapping, and the recorded
> Mediation direction — is in `Atlas-Design-Note-Composition-0.1.md`. None of it enlarges
> Atlas Base.
...
- Interchangeability is a compatibility relationship between Components; hot-swappability is
  ...
  names.
- The proposed **Atlas Portable Binding** is a first-party default seam for Component
  interchange, not the implementation model of Atlas Base. A **Binding Plan** fixes
...
> `Atlas-Design-Note-Persistent-Information-0.1.md`. None of it enlarges Atlas Base, ratifies
...
(§18.2). Their exact compatibility and migration rules remain provisional in Architecture 0.7;
...
Architectures 0.2 through 0.5 are retained in `Atlas-Architecture-Change-History.md`.

The 0.7 revision is in progress and governed by `Atlas-Architecture-0.7-Change-Plan.md`.
...
- **§16.6, §18.1, and §18.2 — Work-in-progress material extracted into companion design
  ...
  Corpus/Dataset/Store/Router now live in `Atlas-Design-Note-Enrichment-0.1.md`,
  `Atlas-Design-Note-Composition-0.1.md`, and
  `Atlas-Design-Note-Persistent-Information-0.1.md`. The architecture document retains each
