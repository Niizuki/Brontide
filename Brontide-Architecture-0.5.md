...
**Status:** Early Draft
**Version:** 0.5 (see §35 for changes from Architecture 0.2, §36 for changes from Architecture 0.3,
and §37 for changes from Architecture 0.4)
...
Architecture 0.5 retains eight Base terms:
...
Shape is therefore part of Brontide Base in Architecture 0.3.
...
remains one Shape. Fragment is a subordinate concept within Shape, not a ninth Brontide Base term.
...
latency, failure, authority, and effect semantics and should remain visible as an ordinary Brontide
...
to inspect. Before ratification Brontide must decide at least:
...
Brontide participant or may be described through future `Resource` and `State` extensions. It may
...
Brontide should distinguish five relationships that conventional systems often collapse:
...
Brontide does not assume that a system has one fixed call graph, pipeline, or global topology. A
...
composition. It must not be treated as a universal property of Brontide systems or as proof that every
Enrichment has a route that Brontide can enumerate.
...
of them as the Brontide model, and implementations need not use the same strategy at every boundary.
...
> *Which Brontide relationship gives propagated information a bounded lifetime and an unambiguous
...
a general Brontide construct. No choice is made in Architecture 0.5. Brontide Reference Stack and Brontide Minimal Stack experiments
...
Architecture 0.5 does not define, name, or reserve such a Profile. It should be introduced only
...
### 18.2 Optional system-native services and boxed compositions
...
- State, Persistence, Resource, Transaction, and database integration;
...
system-native Event, State, Identity, Presentation, Web, scheduling, and accelerator Components.
...
available State, Persistence, Transaction, Web, Presentation, Workspace, Identity, or other
contracts together with provider characteristics. One composition may select an in-memory store,
...
architectural caste. Section 18.2 preserves the equally important opposite choice: an application
may remain a box.
...
Architecture 0.5 preserves the current Base, composition, Profile, and implementation directions;
§35 records the changes from 0.2, §36 records the changes from 0.3, and §37 records the changes
from 0.4. The following remain genuinely open.
...
Section 18.2 permits deep application/system composition and an opaque boxed application as equally
valid choices. The portable discovery, preference, dependency-strength, migration, and fallback
contracts for system-provided Event, State, Identity, Presentation, Web, scheduling, and accelerator
facilities remain open. Tooling must show richer support for participating Components without
...
State, identity, Presentation, Workspace, Web, scheduling, observability, compilation, and
acceleration. Applications may adopt these facilities incrementally and may contribute their own
Operations back to the environment. They may also remain boxed, with private authentication,
database, events, rendering, and internal composition. Brontide conformance applies to the contracts
they claim, not to an obligation to expose their interior.
...
- **§6.8 and §30 — Brontide Minimal Stack introduced as an independent full-stack implementation.** Brontide Reference Stack remains
  the first practical showcase. Brontide Minimal Stack is defined as a lean, independently implemented stack whose
  purpose is to test composability and component interchange with Brontide.Reference. Shared conformance is
...
- **§7 and §16 — Shape added to Brontide Base.** Architecture 0.3 identifies eight Base terms. Shape
...
- **§7, §18, and §19 — Base membership separated from prevalence.** Base membership depends on
  necessity to the smallest meaningful Brontide system and the Embedded Test, not expected deployment
...
- **§18.1 — Scale-independent Component terminology introduced as work in progress.** A Component
  is a bounded unit of composition declaring provided and required Brontide contracts. It may range
...
- **§18.1 and §33 — Brontide Portable Binding proposed as the default Component seam.** A scoped
...
- **§22 — Authored qualification made explicit.** Unqualified canonical names are reserved for
  ratified Brontide concepts. Portable non-standard concepts use
...
- **§16.6 — Value propagation introduced for investigation.** Where an explicit bounded route
  ...
  Brontide systems. Parameter threading, carrier structures, contextual storage, attached fragments,
...
- **§16.6 and §33 — Systems are not necessarily topological.** Brontide does not assume a fixed call
  ...
  Brontide relationship that should bound propagated information when direct dynamic invocation means
...
- **§33 — Open questions expanded.** Enrichment, ambient scope, propagation, hidden dependencies,
  ...
  conflict resolution, and experimental validation across Brontide Reference Stack and Brontide Minimal Stack are now tracked
  explicitly.

Architecture 0.4 makes no change to the eight Brontide Base terms and does not ratify Enrichment,
value propagation, the Component model, or the proposed Brontide Portable Binding. The additions are
...
- **§6.13 and §18.2 — Optional system-native services and boxed applications defined.** A
  general-purpose Brontide environment may offer Event, persistence, State, Identity, Presentation,
  Workspace, Web, scheduling, observability, compilation, and accelerator Components. Applications
  may adopt them incrementally, supply private alternatives, expose only a narrow Brontide boundary,
...
- **§30.1 — A decisive Reference/Minimal demonstration framed.** The proposed collaborative
  image-processing workspace begins as a small local CPU composition, adopts system services
  incrementally, substitutes providers visibly, interchanges Brontide Reference Stack and Brontide Minimal Stack Components, and moves
  an explicitly eligible transformation to a GPU or other accelerator without changing its semantic
  Operation. The final mixed-stack workflow is intended to prove that Brontide is neither Brontide Reference Stack nor
  Brontide.Minimal.
...
- **§33 — Open questions expanded.** System-service discovery and dependency strength,
  ...
  are now tracked explicitly for Reference/Minimal experimentation.
...
Architecture 0.5 makes no change to the eight Brontide Base terms. It does not ratify a system-service
Profile, execution-explanation model, optimisation-property vocabulary, Enrichment, value
propagation, Component model, or Brontide Portable Binding. The new material constrains the larger
