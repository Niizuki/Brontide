...
| Host authority | Brontide Reference Stack `AuthorityDomain` evaluates Actor, Capability, target, Operation, Shape, and Constraints before the handler starts | Brontide Minimal Stack `World.step` evaluates Actor, grant, Operation, Shape, and Constraints before the handler starts | Binding requirement | No Capability is serialized |
