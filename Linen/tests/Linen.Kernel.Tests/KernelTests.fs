namespace Linen.Kernel.Tests

open System
open NUnit.Framework
open Linen.Model
open Linen.Extensions.Events
open Linen.Extensions.Flow
...
type NativeSemanticsTests() =
    ...
    member _.``flow fan-out waits and fan-in becomes ready once dependencies complete`` () =
        let operation: OperationReference =
            { Name = name "linen.tests.flow-operation"
              Version = 1 }

        let acquire = name "linen.tests.acquire"
        let cool = name "linen.tests.cool"
        let inspect = name "linen.tests.inspect"
        let finish = name "linen.tests.finish"
    ...
    member _.``flow definitions reject cycles`` () =
        let operation: OperationReference =
            { Name = name "linen.tests.operation"
              Version = 1 }

        let a = name "linen.tests.a"
        let b = name "linen.tests.b"

        let result =
            Flow.tryCreate
                (name "linen.tests.cycle")
