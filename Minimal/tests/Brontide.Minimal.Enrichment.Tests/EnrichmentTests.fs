namespace Brontide.Minimal.Enrichment.Tests
...
module private Helpers =
    ...
    let fragment () : FragmentReference =
        { Name = name "brontide-minimal.tests.fragment"
          Version = 1 }

    let vocabulary () = name "brontide-minimal.tests.vocabulary"

    let provider () : EnrichmentProvider =
        { Name = name "brontide-minimal.tests.provider"
          Vocabularies = Set.singleton (vocabulary ())
          Resolve =
            fun request ->
                Ok
                    [ { Provider = name "brontide-minimal.tests.provider"
                        Fragment = fragment ()
                        Value = TextValue request.Subject
                        Claims = [ name "brontide-minimal.tests.enriched", request.Subject ] } ] }
...
type EnrichmentTests() =
    ...
    member _.``providers are explicit and selected by name`` () =
        ...
        Assert.That(enrichment.Head.Provider |> CanonicalName.value, Is.EqualTo "brontide-minimal.tests.provider")
    ...
    member _.``unsupported vocabularies are not silently inferred`` () =
        ...
        let request =
            { Subject = "subject-1"
              Vocabulary = name "brontide-minimal.tests.unsupported"
              Input = UnitValue }
    ...
    member _.``pointer temperature Enrichment is targeted local additive and route free`` () =
        ...
        let declaration: TargetedEnrichmentDeclaration =
            { Name = name "brontide-minimal.enrichment.thermal-from-telemetry"
              Target = pointerMove
