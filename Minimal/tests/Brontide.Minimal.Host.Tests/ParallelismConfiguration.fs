module Brontide.Minimal.Host.Tests.ParallelismConfiguration

open NUnit.Framework

// Every test in this assembly owns what it touches: a temporary directory named by a fresh
// identifier, the provider processes it starts and stops, and the fakes it constructs. The only
// mutable state at class level lives in private fakes a test builds for itself, never in a fixture,
// so the tests run together. The process category is where the wall clock went -- each of its tests
// spawns and waits on provider processes -- and run one at a time it was the floor under the whole
// repository gate.
//
