...
- `Brontide.Minimal.Experimental.ComponentManagement` is the fake, deterministic Component Manager
  ...
  values, and an all-false lifecycle/authority effect observation. CM2 adds Minimal-native,
...
- `Brontide.Minimal.Host` composes the independent Component Management and Portable Binding
  experiments for CBI1-CBI27. It maps one completed native CM2 direct `1..1` position into PB7
  ...
  treated as authority for a portable Operation. CBI4 independently serializes five native
  outcomes to shared canonical profile digests; this is data-only parity rather than an integrated
  process seam. CBI5 revalidates that exact local relationship and grant after activation,
  retiring the PB7 member before further ordinary interaction when authority is not renewed. CBI6
  admits a set of participants holding one or more exact narrow grants each over that one member,
  owning the cross-request identity and receiving-domain Actor rules a single CM5 request cannot
  see. CBI7 revalidates that set and retires the shared member when the identical set does not
  renew identically, naming the unrenewed participants instead of narrowing the set. CBI8 grows
  that set in place while the member stays released and declines every change that would shrink it.
  CBI9 removes and substitutes participants under a dependency the resolved definition declares,
  admitting a revision only while every declared dependency stays covered. CBI10 verifies that
  declaration against observed portable interaction through CM4 binding exercises whose authority
  admission is derived rather than claimed. CBI11 narrows a declaration to a successor resolution of
  ...
  restart scope. CBI15 revises those sets per member and checks the result against the activation.
  CBI16 verifies every member's declaration against that member's observed interaction through one
  CM4 request, so one member's undeclared use condemns the activation. CBI17 narrows those
  declarations to one successor generation as a single transaction, so a member the successor does
  not resolve blocks the others. CBI18 grows those sets without consulting any declaration, because
  ...
  retained members only after cutover. CBI20 lets that successor resolve a different set of positions,
  ...
  Mediation is realized as, CBI26 admits that mediator's own authority, and CBI27 carries a position
  wider than `1..1` into preflight as one ordinary member per resolved member.
  It does not merge the projects, perform a lifecycle handshake, detect an attachment it was not given,
  admit authority on behalf of a mediated member, activate a fanned-out Provider Set, or establish real
...
- `Brontide.Minimal.Binding` (`Portable/`) is the Minimal native realization of the
  ...
  fixtures over that layer rather than definitions of it. Its PB4 phase measured the two
  realizations against each other over every portable result class a host can reach, and closed the
...
GPU execution is both **experimental** and a **sideline project**. It is not implemented by the
current milestone path, is not required for Brontide Minimal Stack conformance, and cannot be counted as completion
of Composition, Imaging, or the mixed-stack workspace. CPU execution is the required reference
path. Any future GPU provider must declare eligibility, lowering failures, representation and copy
costs, fallback, interruption, and provider selection; it must remain replaceable without changing
the semantic image Operation.
