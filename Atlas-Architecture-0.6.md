...
**Status:** Early Draft
**Version:** 0.6 (see §35 for changes from Architecture 0.2, §36 for changes from Architecture 0.3,
§37 for changes from Architecture 0.4, and §38 for changes from Architecture 0.5)
...
Atlas should allow a Component to be simple even when the environment around it is sophisticated.
...
A general-purpose Atlas environment may offer persistence, Event Distribution, identity,
...
scheduler, or other private machinery. It may expose only a narrow Atlas boundary, or no internal
Atlas composition at all, and remain a valid application in the surrounding environment.
...
from durable replay, and one that specifically requires a Fabric persistence contract make three
...
Atlas may make substitution and relocation possible while still allowing a developer to answer
...
inside the application's installation or private directory. Atlas must permit that compatibility
...
Architecture 0.6 retains eight Base terms:
...
Architecture 0.6 does not define, name, or reserve such a Profile. It should be introduced only
...
An **Attribute** is a value exposed through a specified Atlas Operation and used declaratively to
...
supplied the values, and which values came from defaults. This explanatory record is part of Atlas's
...
### 18.2 Work in progress: Corpus, Dataset, Store, and Router

Atlas needs a first-class account of persistent information without turning one filesystem,
...
1. **App-Level information** remains privately managed by conventional software outside the
   Corpus model. Atlas may see only files, directories, sandbox storage, or external Operations.
   ...
   implementations.
2. An **Opaque Corpus** defines Atlas-visible Datasets whose internal content Atlas cannot
   interpret through Shapes. Atlas can still manage identity, Store bindings, authority,
...
the canonical Corpus name `Linen:Mail.Accounts` states that `Linen` owns and versions that
definition. It does not state that Linen owns, stores, may access, or exclusively manages every
...
Corpus versions follow the existing Atlas separation between name and version (§23):

```
corpus-name: Linen:Mail.Accounts
corpus-version: 3
owner: Linen
```

`Linen:Mail.Accounts@3` is permitted as explanatory shorthand only if a future notation
...
of objects. Atlas manages the Dataset as a whole without addressing its internal values.
...
- **Record** — one structured root value described by a Shape. Nested fields and sequences do not
  make it several Records; Atlas treats the root as one logical unit.
...
Forms merely to advertise a storage engine. A later Atlas specification revision may add a Form
...
Corpus:
    ...
    form:
        Graph:
            node-shape: Fabric:Editor.ProjectNode 2
            edge-shape: Fabric:Editor.ProjectEdge 1
...
may use implementation-internal physical redundancy without exposing every disk as an Atlas Store.
If independently governed placement, failure, authority, or retention must be visible to Atlas, it
...
Atlas, a Capability is a particular target-recognised grant, not the name of a feature (§10). A
...
work-in-progress composition terms in Architecture 0.6. They do not enlarge Atlas Base, ratify a
Storage vocabulary, require a database, or make Structured integration mandatory. Fabric and Linen
...
A general-purpose Atlas operating environment may participate much more deeply in application
...
Atlas also permits a **boxed composition**: an application whose database, identity, event system,
renderer, scheduler, and internal modules remain private. The box may expose a narrow set of Atlas
...
- **Constraint types** — the Constraints it defines for use with the Base algebra (§10.1), including
  ...
  standardises for composition (§18.1).
...
An Atlas document or machine-readable definition SHOULD declare its **notation strictness**:
...
(§18.2). Their exact compatibility and migration rules remain provisional in Architecture 0.6;
...
Both are legitimate hosted applications. Atlas conformance applies only to the boundaries and
...
The most useful Atlas demonstration should not merely show that a small application can be built.
...
metadata implementation. At least one Fabric Component should be replaced by a Linen Component and
...
The resulting workflow should contain Fabric, Linen, and third-party Components in one composition,
for example a Fabric scheduler, Linen Event store, Fabric accelerator provider, Linen identity
...
Its final claim is therefore not that Fabric is modular. It is that Atlas is not Fabric, and Linen
is not Atlas either.
...
observability, and distributed coordination. Atlas creates the possibility of exposing these as
...
Atlas asks developers to accept composition across more boundaries than conventional application
...
persistence, or stale presentation. An Atlas environment should provide a structured explanation
...
on nondeterminism. Atlas should accelerate implementations that declare and satisfy the relevant
...
Architecture 0.6 preserves the current Base, composition, Profile, and implementation directions;
§35 records the changes from 0.2, §36 records the changes from 0.3, and §37 records the changes
from 0.4. Section 38 records the changes from 0.5. The following remain genuinely open.
...
**Attributes and recursive Definition Constraints.**
Section 18.1 replaces free-floating selection labels with values obtained through exact Atlas
...
Constraints; Fabric and Linen must prove deterministic evaluation and explanation of nested branch
...
annotations carry any machine semantics remain open. Architecture 0.6 does not admit a hidden
...
explanation model, but Atlas does not yet define the minimum trace that answers what happened. The
...
private application mechanism to become Atlas-visible.
...
open. Fabric and Linen should demonstrate the same semantic transformation through CPU and GPU paths
...
becoming part of a canonical name. Architecture 0.6 itself uses NonStrict explanatory notation.
...
outside the model for compatibility; an Opaque Corpus gives Atlas lifecycle understanding without
...
private authentication, database, events, rendering, and internal composition. Atlas conformance
...
Fabric and Linen Components, and moves an explicitly eligible transformation to an accelerator
