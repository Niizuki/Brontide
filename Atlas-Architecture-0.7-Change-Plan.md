...
**Baseline:** Atlas-Architecture-0.6.md
...
`Fabric:Editor.Project.Store.Core`. This collides with legitimate concept paths — nothing forbids
...
```
Atlas-Design-Note-Enrichment-0.1.md
Atlas-Design-Note-Composition-0.1.md
Atlas-Design-Note-Persistent-Information-0.1.md
```
...
- Its three separable responsibilities may live in different places within one system:
  ...
  **evaluation** (§10.1, §13.5) is decentralised to targets by design — Atlas has no central
...
- Capabilities do not travel between trust boundaries; authorisation happens at each boundary.
  The Fabric/Linen interchange rule that no Capability crosses the process seam is this
...
explicit, inspectable rules. It follows the Atlas relationship-noun grammar (Delegation,
...
first instantiation and the Fabric/Linen storage evidence gate is unchanged. The Event
...
semantics wait for Fabric/Linen storage evidence. Tracked in §33.

---

## 3. Pacing and implementation documents

Persistence expansion beyond the C4/C5 rectifications is held until Fabric and Linen produce
...
```
Fabric-Implementation-Plan-0.3.md   — targets Architecture 0.7; adds first Corpus/Dataset/
...
Linen-Implementation-Plan-0.3.md    — independent counterpart; must implement C1 evaluation
...
- **Executed:** C6 (extraction — three design notes, change-history document,
  `Atlas-Architecture-0.7.md` draft with summary stubs, §7.1 term status registry, and a new
