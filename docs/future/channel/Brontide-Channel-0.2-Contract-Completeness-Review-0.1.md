...
| recipient-side frameless refusal after the request crossed dispatch (authority denial, phase refusal, or drain refusing a still-admitting interaction) | the initiator stays `dispatched` with no terminal fact until an out-of-core timer or a transport loss observation closes it as `lost` with `unknown` certainty; a held cancellation control discarded on that branch leaves it in `cancel-pending` for the same time -- **BL5**, a stated limit under the 2026-09-25 ruling | host timer, outside core |
...
| --- | --- | --- | --- |
| S1 | in each endpoint's local history of each session the vector carries, every accepted transition of that session is in the legal table | named negative probe in the neutral verifier, now named and executable as `S1-illegal-transition-accepted` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for, and a property reading one session's fact across the vector is green on the first and red on it; and one session with both endpoints' local histories recorded, the initiator admitting under its own established session after the recipient has begun drain -- the member **BL3** was raised for, on which a property reading the session and not the endpoint is red |
| S2 | no interaction dispatches outside the `established` state of its own session at the endpoint that dispatches it | named negative probe in the neutral verifier, now named and executable as `S2-dispatch-while-draining` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for, and a property reading one session's fact across the vector is green on the first and red on it; and one session with both endpoints' local histories recorded, the initiator admitting under its own established session after the recipient has begun drain -- the member **BL3** was raised for, on which a property reading the session and not the endpoint is red |
| S3 | within each endpoint's local history of each session the vector carries, no new interaction is admitted after that endpoint's first drain transition of that session | named negative probe in the neutral verifier, now named and executable as `S3-admit-after-drain` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for, and a property reading one session's fact across the vector is green on the first and red on it and the session scope is **AL1**: the property bounded admission by "the first drain transition" with no session named, which is not a fact of a vector that carries two, so a second session establishing and admitting after the first one drained took it red on conforming behaviour. The AK audit reported `S1`-`S6` clean because its trigger set was C12's declared fact list and the session's own state was absent from it (**AL3**); and one session with both endpoints' local histories recorded, the initiator admitting under its own established session after the recipient has begun drain -- the member **BL3** was raised for, on which a property reading the session and not the endpoint is red |
| S4 | within each endpoint's local history of each session the vector carries, a terminal session never becomes nonterminal or resumes under the same session identity | named negative probe in the neutral verifier, now named and executable as `S4-terminal-session-resumed` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for, and a property reading one session's fact across the vector is green on the first and red on it; and one session with both endpoints' local histories recorded, the initiator admitting under its own established session after the recipient has begun drain -- the member **BL3** was raised for, on which a property reading the session and not the endpoint is red |
| S5 | for each session the vector carries, at each endpoint, fixed and negotiated establishment of that session's own declared profile produce equal normative profile records | named negative probe in the neutral verifier, now named and executable as `S5-fixed-and-negotiated-differ` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for, and a property reading one session's fact across the vector is green on the first and red on it and the qualifier is **AL4**: `established profile` is a declared per-session fact, so a property comparing "equal normative profiles" across the vector is red on two sessions that legitimately declare different ones. It is the correction `C1-P1` received under AK8, one artifact over |
| S6 | in any endpoint's local history of any session the vector carries, no session event creates Ready, Release, authority, or an application Outcome | named negative probe in the neutral verifier, now named and executable as `S6-session-event-creates-outcome` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for, and a property reading one session's fact across the vector is green on the first and red on it |
...
**BF1**-**BF12** were raised by the twentieth author-side pass, and two of them reach this document,
which is why the family is recorded here whole on AR's, AT's and AU's precedent. The pass built the
instrument the nineteenth had named and left: every field of a recording copy of each declared vector
...
`C3-P1` compares class, direction and phase against the profile through a `profileMatch` Boolean the
vector asserts. `C10-P1`'s completeness clause read a second asserted Boolean; it now reads the
direction itself as an obligation, and the audit row above names `C10-observation-omits-direction`.
**BF4** is the `unseen` refusal record's effect certainty, which C10's owned fact fixes at `known-none`
and which no property read, so `unknown` there was inside the three-member set the design states and
outside the fact the design states. `I4`'s first clause -- every pre-dispatch refusal is `known-none`
-- now quantifies over that record too, the audit row names `I4-unseen-refusal-not-known-none`, and
the conforming refusal at `unseen` joins the property's inputs so the clause is observed staying green.

**BF3** is the one to weigh, and it reaches no design artifact because the design was right. The
brief's vector format says every vector states the initial session state of each session it carries,
and the corpus does -- every C4 vector carries a session that begins `established` with no
establishment in its timeline. `S2` and `C2-P1` began every session at `unestablished` by a literal in
the evaluator, so a conforming realization whose window opens mid-session was red on both, which is
AE1's class: a property red on legal behaviour nobody had written down. Both read the record now, the
legal input joins both properties, and the harness reconciles the stated state against the timeline's
first transition. **BF5**, **BF6**, **BF7** and **BF8** are second surfaces of facts a property reads elsewhere -- the
timeline's copy of the terminal history, the latch's copy of its reference's identity, both
`recordedBy` fields, and the per-interaction copy of the vector's deterministic-observation fact --
each deleted or reconciled the way the interaction record's `dispatched` flag already was. **BF9** is the declared profile, which the brief requires and
`S5` now names in its witness; the comparison against the profile's own declaration remains the
owner-sized unit the nineteenth pass named.

**BF1**, **BF10**, **BF11** and **BF12** are verification rather than design and are recorded here
because the family is classified whole. BF1 is what the frozen set reported: the design gate's AJ2
narrative guard keyed on a bare ordinal with the noun `pass` excluded, the nineteenth pass's narrative
counted clean frozen sets -- "thirteenth consecutive" -- and the probe that pins that guard returned pass at the head,
in a corpus that pass had merged without running. The key now names what introduces a review. BF10
and BF12 are the pass's own code caught by frozen instruments, the read-provenance census and the
coverage measure's operand unit; BF11 is the design gate's next-pass ordinal check, whose word list
ended at twenty and whose ordinal branch skipped silently where its cardinal twin failed loudly, found
by reading the guard the pass had to extend.

**BI1**-**BI3** were raised by the twenty-third author-side pass, and the first reaches this document,
which is why the family is recorded here whole on BF's precedent. The pass sharpened the operand
census the twenty-second built -- every field a property reads, given a wrong value of its own kind --
from the field to the property: a field one property decides on and another only reads is decisive in
the corpus and inert for the second property, and the field-level verdict cannot tell. Every field a
property reads on its own declared inputs must decide that property's verdict on one of them, or be
named in its witness. **On its first run it reported three (property, field) pairs.**

**BI1** is `I5`'s terminal handling. `I5` reads whether a terminal step was accepted and which admitted
identities it closes, and on every input `I5` declared neither ever decided the verdict: each
required-green member holds one interaction against a bound of two, and the one named mutation admits
two before any terminal arrives. An evaluator that freed a slot on any terminal fact, refused or not,
whatever it closed, would have passed every input the property declares -- AU1's unfalsifiable
obligation one operand down. Writing the two mutations found the evaluator wrong as well: it counted
terminal facts rather than tracking the identities they close, so a terminal for `i1` that closed `i2`
freed `i1`'s slot and the property was green on a session holding two against a bound of one. The
audit row above names `I5-refused-terminal-holds-slot` and `I5-terminal-closes-other-identity`, one per
operand, and the evaluator tracks the set of admitted identities no accepted terminal has closed.
**BI2** is `I6`'s witness naming the interaction identity alone, where one identity may be open in two
sessions at once; it names the session now, and reaches no design artifact. **BI3** is the future-work
index's Channel row naming the next pass the twenty-first for three passes, found by reading while the
