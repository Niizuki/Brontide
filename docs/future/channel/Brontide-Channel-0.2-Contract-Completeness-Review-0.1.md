...
| C2 | C2-P1 legal transition/terminal monotonicity | accept new interaction while draining, executable as `C2-accept-interaction-while-draining` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for |
| C3 | C3-P1 exact class/direction/phase | mark unknown phase as true, executable as `C3-unknown-phase-marked-true` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for |
...
| C5 | C5-P1 all positional/bound checks before dispatch | project an authority value or dispatch oversized payload, executable as `C5-oversized-payload-dispatched` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for |
| C6 | C6-P1 exact permitted local authority | treat compatibility/delivery as permission, executable as `C6-delivery-treated-as-permission` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for |
| C7 | C7-P1 exact declaration/pre-Ready/no phase creation | admit wrong edge or let success create Ready, executable as `C7-success-creates-ready` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for |
| C8 | C8-P1 one terminal; controls never success | make cancel acknowledgement terminal success, executable as `C8-acknowledgement-recorded-as-success` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for |
| C9 | C9-P1 exactly one provenance form | map local loss to peer fault, executable as `C9-local-loss-mapped-to-peer-fault` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for |
| C10 | C10-P1 no unsupported known-none after dispatch | replace unknown effect certainty with known-none, executable as `C10-known-none-after-dispatch` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for |
| C11 | C11-P1 required facets exact/core invariants stable | extension changes interaction identity or authority result, executable as `C11-facet-changes-interaction-identity` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for |
...
| I4 | every pre-dispatch refusal is `known-none`; every possible post-dispatch loss is `unknown` | `I4-post-dispatch-loss-known-none`, executable in `build/verify-channel-0.2-properties.ps1` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for, and a property reading one session's fact across the vector is green on the first and red on it |
...
| I6 | a relational interaction matches exactly one declaration and never creates Ready/Release | `I6-relational-creates-ready`, executable in `build/verify-channel-0.2-properties.ps1` | the conforming single-session realization, and two sessions conforming, the second establishing and admitting after the first drains -- the second is the member AL1 and AK7 were raised for, and a property reading one session's fact across the vector is green on the first and red on it |
