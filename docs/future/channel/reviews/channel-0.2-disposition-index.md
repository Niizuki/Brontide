...
Under **BL1**-**BL12**, closure review 17's corrections: C2 counts drain per endpoint (**BL1**) and `C2-P1` names the endpoint's local history (**BL3**); C4 states session-control order, scopes its silence to match, and asks the profile to declare it (**BL2**); C11 names session-control order among the ordering facts core owns (**BL2**); C12's `session state` entry is each endpoint's own (**BL3**); C10 names the initiator's frame for an identity it never opened (**BL12**); and the status block no longer describes the review as on hold (**BL6**).
...
Under **BL1**-**BL12**: the legal table and drain protocol count drain per endpoint, with a `draining` row for the peer's first drain after this endpoint's own (**BL1**); the close and drain rules state the session-control order they rely on (**BL2**); every capability-wide property names the endpoint's local history of its session (**BL3**); and the status block no longer describes the review as on hold (**BL6**).
...
Under **BL1**-**BL12**: the recipient's `executing` row and held-control row emit the `accepted` acknowledgement (**BL4**); a recipient-side frameless refusal after dispatch is stated as a limit owned by the host timer (**BL5**); the initiator drops a frame naming an identity it never opened, with the observation C10 requires (**BL12**); and the status block no longer describes the review as on hold (**BL6**).
...
Under **BL1**-**BL12**: the session grid's `draining` drain cell routes the peer's first drain to `draining` and only a second from the same peer to `faulted` (**BL1**); the recipient `executing` cancellation cell emits the `accepted` and `refused` acknowledgements (**BL4**); and the status block no longer describes the review as on hold (**BL6**).
...
Under **BL1**-**BL12**: a `Session-control order` row owned by `channel`, and the wire-encoding row's realization declaration names it beside per-interaction frame order (**BL2**); and the status block no longer describes the review as on hold (**BL6**).
...
disposition history now runs to the seventeenth cycle rather than stopping at the fifth — the count is
...
Under **BL1**-**BL12**: the new-evidence inventory lists session-control order and its two reorderings (**BL2**); and the status block no longer describes the review as on hold (**BL6**).
...
Under **BL1**-**BL12**: the establishment rule carries the realization's session-control order declaration (**BL2**); the vector format has every session transition name the endpoint whose local history it belongs to (**BL3**); and the status block no longer describes the review as on hold (**BL6**).
