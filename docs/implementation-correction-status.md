...
| F — independent-review invariant gaps | Additional Minimal Genesis correction in progress; full-gate rerun and fresh pinned reviews required | Reference Genesis/escaped-authority/provenance/liveness tests and matrix; Minimal Fragment-host-lineage and failed-Genesis identity tests and matrix; both changelogs |
...
recycled an escaped Actor identity after a failed persistent-World Genesis branch. That analogue is
being corrected and must pass the complete gate and both repinned reviews before deletion can be
authorized.
